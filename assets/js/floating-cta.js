...
(function(){
  ...
  var html = css +
    '<div id="snt-fab" class="snt-fab" role="region" aria-label="Quick contact">' +
      ...
      '</button>' +
    '</div>' +
    '<div id="snt-wechat-modal" class="snt-wcm" role="dialog" aria-label="Contacter sur WeChat" onclick="if(event.target===this)this.classList.remove(\'is-open\');">' +
      '<div class="snt-wcm__box">' +
        '<button class="snt-wcm__close" onclick="document.getElementById(\'snt-wechat-modal\').classList.remove(\'is-open\');" aria-label="Fermer">×</button>' +
        '<h3 class="snt-wcm__title">Contacter sur WeChat</h3>' +
        '<p class="snt-wcm__desc">Scannez le QR ou cherchez notre WeChat ID:</p>' +
